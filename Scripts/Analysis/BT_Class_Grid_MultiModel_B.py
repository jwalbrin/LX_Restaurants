...
Version B implements RFE, and therefore grid search is performed
as last step in pipeline with RFE + classifier as estimator"""
...
feats_to_keep = "All" # "All" or np.array of indices
...
"""Pipe and params; note that params are passed directly as a dictionary 
to gridsearchCV"""

pipe_params = [
              Pipeline([
              ('scaler', StandardScaler()),              
              ('clf', GridSearchCV(RFE(
              ...
              ('clf', GridSearchCV(RFE(
                      GradientBoostingClassifier(random_state = 42),
                      n_features_to_select = 25, step= 10), 
                    {'estimator__n_estimators': [100, 300, 500]},
                    scoring='balanced_accuracy'))
              ]),
              Pipeline([
              ...
              Pipeline([
              ('scaler', StandardScaler()),              
              ('clf', GridSearchCV(RFE(LogisticRegression(random_state=42),
                      n_features_to_select = 50, step= 10), 
                    {'estimator__C': [1, 10, 20, 50]},
                    scoring='balanced_accuracy'))
              ]),
              Pipeline([
              ('scaler', StandardScaler()),              
...
for m_i, m in enumerate(rep_models):    
    for ts_i, ts in enumerate(tr_splits):
        ...
        for rc_i, rc in enumerate(rc_vals):            
            ...
            #--- Pipe-grid-fit              
            for pp_i, pipe in enumerate(pipe_params):
                ...
                e_name = (str(pipe.get_params("clf")["clf"]).split(
                                                    "(estimator=")[1] + "_" +
                         str(pipe.get_params("clf")["clf"]).split(
                                 "estimator=")[2].split("()")[0])
...
x_labels = list(np.arange(0,len(df_out)))
